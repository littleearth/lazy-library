...
uses
  ...
  WinApi.Windows, WinApi.ActiveX, WinApi.VSS.Api;
...
type
  ...
  private
    ...
    IsSelectable, NotifyOnBackupComplete, IsTopLevel, IsExcluded,
  ...
  public
    ...
    procedure Initialize(const WriterNameParam: string;
      pComponent: IVssWMComponent)overload;
    // Initialize from a IVssComponent
    procedure Initialize(const WriterNameParam: string;
      pComponent: IVssComponent)overload;
    ...
    function GetStringFromComponentType(eComponentType
      : TVssComponentType): string;
  ...
  public
    ...
    procedure InitializeComponentsForRestore(WriterComponents
      : IVssWriterComponentsExt);
    // Print summary/detailed information about this writer
    procedure Print(bListDetailedInfo: boolean);
    function GetStringFromRestoreMethod(eRestoreMethod
      : TVssRestoreMethodEnum): string;
    function GetStringFromRestoreConditions(eRestoreEnum
  ...
  private
    ...
    FLatestSnapshotIdList: TGuidList;
    // List of shadow copy IDs from the latest shadow copy creation process
    FLatestSnapshotSetID: TGuid; // Latest shadow copy set ID
    ...
    function DisplayNameForVolume(const volumeName: string): string; overload;
    ...
    procedure DiscoverDirectlyExcludedComponents(ExcludedWriterAndComponentList
      : TStringList; AWriterList: TObjectList);
    procedure DiscoverNonShadowedExcludedComponents(ShadowSourceVolumes
      : TStringList);
    ...
    procedure VerifyExplicitelyIncludedComponent(const AIncludedComponent
      : string; AWriterList: TObjectList);
  ...
  public
    ...
    procedure SelectComponentsForBackup(ShadowSourceVolumes,
      ExcludedWriterAndComponentList, IncludedWriterAndComponentList
      : TStringList);
    procedure CreateSnapshotSet(AVolumeList: TStringList;
      AExcludedWriterList: TStringList = nil;
      AIncludedWriterList: TStringList = nil;
      AOutXmlFile: string = ''); overload;
    ...
    procedure GetSnapshotProperties(SnapshotSetID: TGuid;
      var ASnap: TVssSnapshotProp);
    procedure PrintSnapshotProperties(Prop: TVssSnapshotProp);
    procedure FreeSnapshotProperties(ASnap: TVssSnapshotProp);
    ...
    property LatestSnapshotSetID: TGuid read FLatestSnapshotSetID;
    property LatestSnapshotIdList: TGuidList read FLatestSnapshotIdList;
  end;
  ...
  private
    ...
    FDrive, FShadowDeviceName: string;
    FSuccess: boolean;
...
var
  VssCoInitializeSecurityCalled: boolean;
...
// Get the unique volume name for the given drive
...
function TryStringToGUID(const s: string; AGuid: TGuid): boolean;
...
procedure VssCoInitializeSecurity;
begin
  // Only call this once in a process life time.
  OleCheck(CoInitializeSecurity(nil,
    ...
    COMAdminAuthenticationPacket, // Strongest COM authentication level
    COMAdminImpersonationImpersonate, // Minimal impersonation abilities
    nil, // Default COM authentication settings
    COMAdminAuthenticationCapabilitiesNone, // No special options
    nil // Reserved parameter
    ));
  VssCoInitializeSecurityCalled := true;
end;
...
begin
  if (length(Drive) > 0) and GetVolumeNameForVolumeMountPoint(PChar(Drive),
    uname, MAX_PATH + 1) then
    Result := uname
  else
    Result := '';
end;
...
begin
  ...
  if ClusterIsPathOnSharedVolume(PWideChar(path)) then
  begin
...
var
  dwRequired: cardinal;
  volumeMountPoints: array [0 .. MAX_PATH] of WideChar;
  i, n: integer;
begin
  Result := false;
  ...
  if not GetVolumePathNamesForVolumeName(PWideChar(volumeName),
    @volumeMountPoints, MAX_PATH, dwRequired) then
  begin
    // If not enough, retry with a larger size
    // SetLength(volumeMountPoints, dwRequired);
    if (dwRequired = 0) and not GetVolumePathNamesForVolumeName
      (PWideChar(volumeName), @volumeMountPoints, MAX_PATH, dwRequired) then
      Exit;
  end;
  ...
  i := 0;
  repeat
    if volumeMountPoints[i] <> #0 then
    begin
      n := i;
      repeat
        inc(i)
      until volumeMountPoints[i] = #0;
      if (i - n) < length(volumeNameCanon) then
      begin
...
begin
  case hrError of
    VSS_E_BAD_STATE:
      Result := 'VSS: called function was in incorrect state';
    VSS_E_UNEXPECTED:
      Result := 'VSS: an unexpected error was encountered';
    VSS_E_PROVIDER_ALREADY_REGISTERED:
      Result := 'VSS: provider has already been registered';
    VSS_E_PROVIDER_NOT_REGISTERED:
      Result := 'VSS: provider is not registered';
    VSS_E_PROVIDER_VETO:
      Result := 'VSS: provider had an error';
    VSS_E_PROVIDER_IN_USE:
      Result := 'VSS: provider is currently in use';
    VSS_E_OBJECT_NOT_FOUND:
      Result := 'VSS: specified object was not found';
    VSS_S_ASYNC_PENDING:
      Result := 'VSS: asynchronous operation is pending';
    VSS_S_ASYNC_FINISHED:
      Result := 'VSS: asynchronous operation has completed';
    VSS_S_ASYNC_CANCELLED:
      Result := 'VSS: asynchronous operation has been cancelled';
    VSS_E_VOLUME_NOT_SUPPORTED:
      Result := 'VSS: specified volume is not supported';
    VSS_E_VOLUME_NOT_SUPPORTED_BY_PROVIDER:
      Result := 'VSS: provider does not support the specified volume';
    VSS_E_OBJECT_ALREADY_EXISTS:
      Result := 'VSS: object already exists.';
    VSS_E_UNEXPECTED_PROVIDER_ERROR:
      Result := 'VSS:  unexpected provider error';
    VSS_E_CORRUPT_XML_DOCUMENT, VSS_E_INVALID_XML_DOCUMENT:
      Result := 'VSS: XML document is invalid';
    VSS_E_MAXIMUM_NUMBER_OF_VOLUMES_REACHED:
      Result := 'VSS: maximum number of volumes reached';
    VSS_E_FLUSH_WRITES_TIMEOUT:
      Result := 'VSS: provider timed out while flushing data';
    VSS_E_HOLD_WRITES_TIMEOUT:
      Result := 'VSS: provider timed out while holding writes';
    VSS_E_UNEXPECTED_WRITER_ERROR:
      Result := 'VSS: problems while sending events to writers';
    VSS_E_SNAPSHOT_SET_IN_PROGRESS:
      Result := 'VSS: Another shadow copy creation is already in progress';
    VSS_E_MAXIMUM_NUMBER_OF_SNAPSHOTS_REACHED:
      Result := 'VSS: maximum number of shadow copies reached';
    VSS_E_WRITER_INFRASTRUCTURE:
      Result := 'VSS: problem occurred while trying to contact writers';
    VSS_E_WRITER_NOT_RESPONDING:
      Result := 'VSS: writer did not respond to a GatherWriterStatus';
    VSS_E_WRITER_ALREADY_SUBSCRIBED:
      Result := 'VSS: writer has already successfully called the Subscribe function';
    VSS_E_UNSUPPORTED_CONTEXT:
      Result := 'VSS: provider does not support the specified shadow copy type';
    VSS_E_VOLUME_IN_USE:
      Result := 'VSS: specified shadow copy storage association is in use';
    VSS_E_MAXIMUM_DIFFAREA_ASSOCIATIONS_REACHED:
      Result := 'VSS: Maximum number of shadow copy storage associations already reached';
    VSS_E_INSUFFICIENT_STORAGE:
      Result := 'VSS: Insufficient storage available';
    VSS_E_NO_SNAPSHOTS_IMPORTED:
      Result := 'VSS: No shadow copies were successfully imported';
    VSS_S_SOME_SNAPSHOTS_NOT_IMPORTED:
      Result := 'VSS: Some shadow copies were not successfully imported';
    VSS_E_MAXIMUM_NUMBER_OF_REMOTE_MACHINES_REACHED:
      Result := 'VSS: maximum number of remote machines has been reached.';
    VSS_E_REMOTE_SERVER_UNAVAILABLE:
      Result := 'VSS: remote server is unavailable';
    VSS_E_REMOTE_SERVER_UNSUPPORTED:
      Result := 'VSS: that does not support remote shadow-copy creation';
    VSS_E_REVERT_IN_PROGRESS:
      Result := 'VSS: revert is currently in progress';
    VSS_E_REVERT_VOLUME_LOST:
      Result := 'VSS: volume being reverted was lost during revert';
    VSS_E_REBOOT_REQUIRED:
      Result := 'VSS: reboot is required after this operation';
    VSS_E_TRANSACTION_FREEZE_TIMEOUT:
      Result := 'VSS: timeout occurred while freezing a transaction manager';
    VSS_E_TRANSACTION_THAW_TIMEOUT:
      Result := 'VSS: Too much time elapsed between freezing and thawing';
    VSS_E_VOLUME_NOT_LOCAL:
      Result := 'VSS: volume is not mounted on the local host';
    VSS_E_CLUSTER_TIMEOUT:
      Result := 'VSS: timeout occurred while preparing a cluster shared volume';
    VSS_E_WRITERERROR_INCONSISTENTSNAPSHOT:
      Result := 'VSS: shadow-copy set contains only a subset of the volumes';
    VSS_E_WRITERERROR_OUTOFRESOURCES:
      Result := 'VSS: resource allocation failed';
    VSS_E_WRITERERROR_TIMEOUT:
      Result := 'VSS: timeout expired between Freeze and Thaw';
    VSS_E_WRITERERROR_RETRYABLE:
      Result := 'VSS: writer experienced a transient error';
    VSS_E_WRITERERROR_NONRETRYABLE:
      Result := 'VSS: writer experienced a non-transient error';
    VSS_E_WRITERERROR_RECOVERY_FAILED:
      Result := 'VSS: writer experienced an error while trying to recover';
    VSS_E_BREAK_REVERT_ID_FAILED:
      Result := 'VSS: break operation failed, partition identities could not be reverted';
    VSS_E_LEGACY_PROVIDER:
      Result := 'VSS: version of hardware provider does not support this operation';
    VSS_E_MISSING_DISK:
      Result := 'VSS: expected disk did not arrive in the system';
    VSS_E_MISSING_HIDDEN_VOLUME:
      Result := 'VSS: expected hidden volume did not arrive in the system';
    VSS_E_MISSING_VOLUME:
      Result := 'VSS: expected volume did not arrive in the system';
    VSS_E_AUTORECOVERY_FAILED:
      Result := 'VSS: autorecovery operation failed';
    VSS_E_DYNAMIC_DISK_ERROR:
      Result := 'VSS: error processing the dynamic disks';
    VSS_E_NONTRANSPORTABLE_BCD:
      Result := 'VSS: Backup Components Document is non-transportable';
    VSS_E_CANNOT_REVERT_DISKID:
      Result := 'VSS: MBR signature or GPT ID could not be set';
    VSS_E_RESYNC_IN_PROGRESS:
      Result := 'VSS: LUN resynchronization operation could not be started';
    VSS_E_CLUSTER_ERROR:
      Result := 'VSS: clustered disks could not be enumerated';
    VSS_E_ASRERROR_DISK_ASSIGNMENT_FAILED:
      Result := 'VSS: too few disks on this computer';
    VSS_E_ASRERROR_DISK_RECREATION_FAILED:
      Result := 'VSS: cannot create a disk on this computer needed to restore';
    VSS_E_ASRERROR_NO_ARCPATH:
      Result := 'VSS: computer needs to be restarted to finish preparing a hard disk for restore';
    VSS_E_ASRERROR_MISSING_DYNDISK:
      Result := 'VSS: backup failed due to a missing disk for a dynamic volume';
    VSS_E_ASRERROR_SHARED_CRIDISK:
      Result := 'VSS: Automated System Recovery failed';
    VSS_E_ASRERROR_DATADISK_RDISK0:
      Result := 'VSS: data disk is currently set as active in BIOS.';
    VSS_E_ASRERROR_RDISK0_TOOSMALL:
      Result := 'VSS: disk that is set as active in BIOS is too small';
    VSS_E_ASRERROR_CRITICAL_DISKS_TOO_SMALL:
      Result := 'VSS: failed to find enough suitable disks';
    VSS_E_WRITER_STATUS_NOT_AVAILABLE:
      Result := 'VSS: writer status is not available';
    VSS_E_UNSELECTED_VOLUME:
      Result := 'VSS: operation would overwrite a volume that is not explicitly selected';
    VSS_E_SNAPSHOT_NOT_IN_SET:
      Result := 'VSS: shadow copy ID was not found';
    VSS_E_NESTED_VOLUME_LIMIT:
      Result := 'VSS: specified volume is nested too deeply';
    VSS_E_ASRERROR_DYNAMIC_VHD_NOT_SUPPORTED:
      Result := 'critical dynamic disk is a Virtual Hard Disk';
    VSS_E_CRITICAL_VOLUME_ON_INVALID_DISK:
      Result := 'critical volume exists on a disk which cannot be backed up by ASR';
    VSS_E_ASRERROR_RDISK_FOR_SYSTEM_DISK_NOT_FOUND:
      Result := 'No disk that can be used for recovering the system disk found';
    VSS_E_ASRERROR_NO_PHYSICAL_DISK_AVAILABLE:
      Result := 'no fixed disk found that can be used to recreate volumes';
    VSS_E_ASRERROR_FIXED_PHYSICAL_DISK_AVAILABLE_AFTER_DISK_EXCLUSION:
      Result := 'no disk found which it can use for recreating volumes';
    VSS_E_ASRERROR_CRITICAL_DISK_CANNOT_BE_EXCLUDED:
      Result := 'Restore failed because a disk which was critical at backup is excluded';
    VSS_E_ASRERROR_SYSTEM_PARTITION_HIDDEN:
      Result := 'active system partition is hidden';
    // Regular COM errors
    S_OK:
      Result := 'Operation succeeded';
    S_FALSE:
      Result := 'Operation succeeded but returned "FALSE"';
    E_UNEXPECTED:
    ...
    RPC_E_TOO_LATE:
      Result := 'Security must be initialized before any interfaces are marshalled or unmarshalled. It cannot be changed once initialized.';
    RPC_E_NO_GOOD_SECURITY_PACKAGES:
      Result := 'No security packages are installed on this machine or the user is not logged on or there are no compatible security packages between the client and server.';
  else
    begin
      if TLongWord(hrError).Hi and $7FF = FACILITY_WIN32 then
      begin
...
procedure OleErrorHint(ErrorCode: HResult; const Hint: string = '');
begin
  raise EOleSysError.Create(Hint, ErrorCode, 0);
end;

// Raise EOleSysError exception if result code indicates an error }
procedure OleCheck(Result: HResult; AWriteLineToLog: TOutputString;
  Hint: string = '');
begin
  ...
  begin
    if length(Hint) = 0 then
      Hint := '<unknown>';
    if assigned(AWriteLineToLog) then
    begin
...
begin
  ...
  with fs do
  begin
    Write(bstrXML[1], length(bstrXML));
    Free;
  end;
end;
...
begin
  ...
  n := ExpandEnvironmentStrings(PWideChar(path), PWideChar(ExpandedPath),
    MAX_PATH);
  if n > MAX_PATH then
  begin
...
begin
  ...
  begin
    if length(alternatePath) > 0 then
      s := ' ,Alternate Location = ' + alternatePath
    else
      s := '';
    if isRecursive then
      s := ' Recursive : ' + BooleanToString(isRecursive, 'YES', 'NO') + s;
...
// Initialize from a IVssWMComponent
procedure TVssComponent.Initialize(const WriterNameParam: string;
...
begin
  ...
  if not AnsiStartsText('\', FullPath) then
    FullPath := '\' + FullPath;
  with pInfo^ do
  begin
    ...
    if cDatabases > 0 then
      for i := 0 to cDatabases - 1 do
      begin
  ...
  for i := 0 to Descriptors.Count - 1 do
    with Descriptors[i] as TVssFileDescriptor do
    begin
      if AffectedPaths.IndexOf(ExpandedPath) < 0 then
        AffectedPaths.Add(ExpandedPath);
      if AffectedVolumes.IndexOf(AffectedVolume) < 0 then
        AffectedVolumes.Add(AffectedVolume);
    end;
  // Print(true);
end;
...
begin
  ...
  begin
    fWriteLineToLog(Format('    - Component: "%s:%s"' + sLineBreak +
      '      - Name: "%s"' + sLineBreak + '      - Logical Path: "%s"' +
      sLineBreak + '      - Full Path: "%s"' + sLineBreak +
      '      - Caption: "%s"' + sLineBreak + '      - Type: %s [%d]' +
      sLineBreak + '      - Is Selectable: %s' + sLineBreak +
      '      - Is top level: %s' + sLineBreak +
      '      - Notify on backup complete: %s', [WriterName, FullPath, CompName,
      LogicalPath, FullPath, Caption, GetStringFromComponentType(CompType),
      integer(CompType), BooleanToString(IsSelectable, 'Yes', 'No'),
      BooleanToString(IsTopLevel, 'Yes', 'No'),
      BooleanToString(NotifyOnBackupComplete, 'Yes', 'No')]));
    // Compute the affected paths and volumes
    if bListDetailedInfo then
    begin
      fWriteLineToLog('      - Components:');
      for i := 0 to Descriptors.Count - 1 do
        (Descriptors[i] as TVssFileDescriptor).Print;
    end;
    // Print the affected paths and volumes
    fWriteLineToLog('        - Affected paths by this component:');
    for i := 0 to AffectedPaths.Count - 1 do
      fWriteLineToLog(Format('          - %s', [AffectedPaths[i]]));
    fWriteLineToLog('        - Affected volumes by this component:');
    for i := 0 to AffectedVolumes.Count - 1 do
    begin
      if GetDisplayNameForVolume(AffectedVolumes[i], wsLocalVolume) then
        fWriteLineToLog(Format('          - %s [%s]', [AffectedVolumes[i],
          wsLocalVolume]))
      else
        fWriteLineToLog(Format('          - %s [Not valid for local machine]',
          [AffectedVolumes[i]]));
    end;
  end;
end;
...
// Convert a component type into a string
function TVssComponent.GetStringFromComponentType(eComponentType
  : TVssComponentType): string;
begin
  case eComponentType of
    VSS_CT_DATABASE:
      Result := 'VSS_CT_DATABASE';
    VSS_CT_FILEGROUP:
      Result := 'VSS_CT_FILEGROUP';
  else
...
begin
  ...
  if length(descendent.FullPath) <= length(FullPath) then
    Result := false
  else
  begin
    fullPathAppendedWithBackslash := SetPathDelimiter(FullPath);
    descendentPathAppendedWithBackslash :=
      SetPathDelimiter(descendent.FullPath);
    // Return TRUE if the current full path is a prefix of the child full path
    Result := AnsiStartsText(fullPathAppendedWithBackslash,
      descendentPathAppendedWithBackslash);
  end;
end;
...
begin
  ...
  OleCheck(Metadata.GetIdentity(idInstance, idWriter, bstrWriterName, Usage,
    Source), fWriteLineToLog, 'TVssWriter.Initialize:Metadata.GetIdentity');
  // Get the restore method
  OleCheck(Metadata.GetRestoreMethod(RestoreMethod, bstrService,
    bstrUserProcedure, WriterRestoreConditions, RebootRequiredAfterRestore,
    iMappings), fWriteLineToLog,
  ...
  // Get file counts
  OleCheck(Metadata.GetFileCounts(cIncludeFiles, cExcludeFiles, cComponents),
    fWriteLineToLog, 'TVssWriter.Initialize:Metadata.GetFileCounts');
  // Get exclude files
  if cExcludeFiles > 0 then
    for i := 0 to cExcludeFiles - 1 do
    begin
  ...
  begin
    ...
    begin
      ...
      for j := 0 to cComponents - 1 do
        if (Components[j] as TVssComponent).IsAncestorOf(Component) then
          Component.IsTopLevel := false;
    end;
  end;
end;

// Initialize from a IVssWriterComponentsExt
procedure TVssWriter.InitializeComponentsForRestore(WriterComponents
  : IVssWriterComponentsExt);
var
  cComponents: UInt;
  iComponent: integer;
...
begin
  ...
  // Enumerate components
  for iComponent := 0 to Components.Count - 1 do
  begin
    // Get component
    OleCheck(WriterComponents.GetComponent(iComponent, pComponent),
      fWriteLineToLog,
    ...
    if assigned(fWriteLineToLog) then
      fWriteLineToLog(Format('- Found component available for restore : "%s"',
        [fComponent.FullPath]));
...
begin
  ...
  begin
    fWriteLineToLog(Format(sLineBreak + '* WRITER "%s"' + sLineBreak +
      '    - WriterId   = %s' + sLineBreak + '    - InstanceId = %s' +
      sLineBreak + '    - Supports restore events = %s' + sLineBreak +
      '    - Writer restore conditions = %s' + sLineBreak +
      '    - Restore method = %s' + sLineBreak +
      '    - Requires reboot after restore = %s' + sLineBreak,
      [WriterName, WriterId, InstanceId, BooleanToString(supportsRestore, 'YES',
      'NO'), GetStringFromRestoreConditions(WriterRestoreConditions),
      GetStringFromRestoreMethod(RestoreMethod),
      BooleanToString(RebootRequiredAfterRestore, 'YES', 'NO')]));
    // Print exclude files
    fWriteLineToLog('    - Excluded files (' +
      IntToStr(ExcludedFiles.Count) + '):');
    for i := 0 to ExcludedFiles.Count - 1 do
      (ExcludedFiles[i] as TVssFileDescriptor).Print;
    // Enumerate components
    for i := 0 to Components.Count - 1 do
      (Components[i] as TVssComponent).Print(bListDetailedInfo);
  end;
end;
...
begin
  case eRestoreMethod of
    VSS_RME_UNDEFINED:
      Result := 'VSS_RME_UNDEFINED';
    VSS_RME_RESTORE_IF_NOT_THERE:
      Result := 'VSS_RME_RESTORE_IF_NOT_THERE';
    VSS_RME_RESTORE_IF_CAN_REPLACE:
      Result := 'VSS_RME_RESTORE_IF_CAN_REPLACE';
    VSS_RME_STOP_RESTORE_START:
      Result := 'VSS_RME_STOP_RESTORE_START';
    VSS_RME_RESTORE_TO_ALTERNATE_LOCATION:
      Result := 'VSS_RME_RESTORE_TO_ALTERNATE_LOCATION';
    VSS_RME_RESTORE_AT_REBOOT:
      Result := 'VSS_RME_RESTORE_AT_REBOOT';
    VSS_RME_CUSTOM:
      Result := 'VSS_RME_CUSTOM';
    VSS_RME_RESTORE_STOP_START:
      Result := 'VSS_RME_RESTORE_STOP_START';
  else
    Result := 'Undefined';
  end;
end;
...
begin
  case eRestoreEnum of
    VSS_WRE_UNDEFINED:
      Result := 'VSS_WRE_UNDEFINED';
    VSS_WRE_NEVER:
      Result := 'VSS_WRE_NEVER';
    VSS_WRE_IF_REPLACE_FAILS:
      Result := 'VSS_WRE_IF_REPLACE_FAILS';
    VSS_WRE_ALWAYS:
      Result := 'VSS_WRE_ALWAYS';
  else
    Result := 'Undefined';
  end;
end;
...
begin
  ...
  FLatestSnapshotSetID := GUID_NULL;
  DuringRestore := false;
  LogFileName := SetPathDelimiter(TLZFile.GetTempFolder) + defLogFile;
  fWriteLog := false;
...
function TVolumeShadowCopy.GetStringFromWriterStatus(eWriterStatus
  : TVssWriterState): string;
begin
  case eWriterStatus of
    VSS_WS_STABLE:
      Result := 'Writer state : stable';
    VSS_WS_WAITING_FOR_FREEZE:
      Result := 'Writer state : waiting for freeze';
    VSS_WS_WAITING_FOR_THAW:
      Result := 'Writer state : waiting for thaw';
    VSS_WS_WAITING_FOR_POST_SNAPSHOT:
      Result := 'Writer state : waiting for post snapshot';
    VSS_WS_WAITING_FOR_BACKUP_COMPLETE:
      Result := 'Writer state : waiting for backup complete';
    VSS_WS_FAILED_AT_IDENTIFY:
      Result := 'Writer state : failed at identy';
    VSS_WS_FAILED_AT_PREPARE_BACKUP:
      Result := 'Writer state : failed at prepare backup';
    VSS_WS_FAILED_AT_PREPARE_SNAPSHOT:
      Result := 'Writer state : failed at prepare snapshot';
    VSS_WS_FAILED_AT_FREEZE:
      Result := 'Writer state : failed at freeze';
    VSS_WS_FAILED_AT_THAW:
      Result := 'Writer state : failed at thaw';
    VSS_WS_FAILED_AT_POST_SNAPSHOT:
      Result := 'Writer state : failed at post snapshot';
    VSS_WS_FAILED_AT_BACKUP_COMPLETE:
      Result := 'Writer state : failed at backup complete';
    VSS_WS_FAILED_AT_PRE_RESTORE:
      Result := 'Writer state : failed at restore';
    VSS_WS_FAILED_AT_POST_RESTORE:
      Result := 'Writer state : failed at post restore';
  else
    Result := Format('Unknown constant: %d', [integer(eWriterStatus)]);
  end;
end;
...
begin
  if not Result then
    WinCheckError(GetLasterror, Hint);
end;

procedure TVolumeShadowCopy.WinCheckError(LastError: integer;
  Hint: string = '');
begin
  ...
  begin
    if length(Hint) = 0 then
      Hint := '<unknown>';
    WriteLineToLog
      (Format(sLineBreak + 'ERROR : Win32 call "%s" failed.', [Hint]));
...
begin
  WinApi.VSS.Utils.OleCheck(Result, WriteLineToLog, Hint);
end;

function TVolumeShadowCopy.DisplayNameForVolume(const volumeName
  : string): string;
begin
  WinCheck(GetDisplayNameForVolume(volumeName, Result));
end;
...
begin
  if fWriteLog then
  begin
    if not AnsiSameText(Value, fLogFileName) then
    begin
      CloseFile(LogFile);
...
begin
  if Value then
  begin // enable log
    if not fWriteLog then
      fWriteLog := AppendFile(fLogFileName, LogFile);
  end
  else
  begin // disable log
    if fWriteLog then
...
procedure TVolumeShadowCopy.ShowStatusMsg(const s: string);
begin
  if assigned(fShowStatus) then
    fShowStatus(s);
  WriteLineToLog(s);
end;
...
begin
  ...
  begin
    ...
    WriteLineToLog(Format('* SNAPSHOT ID = %s ...',
      [GuidToString(m_SnapshotId)]));
    WriteLineToLog(Format('   - Shadow copy Set: %s',
      [GuidToString(m_SnapshotSetId)]));
    WriteLineToLog(Format('   - Original count of shadow copies = %d',
      [m_lSnapshotsCount]));
    WriteLineToLog(Format('   - Original Volume name: %s [%s]',
      [m_pwszOriginalVolumeName,
      DisplayNameForVolume(m_pwszOriginalVolumeName)]));
    WriteLineToLog(Format('   - Creation Time: %s',
      [VssTimeToString(m_tsCreationTimestamp)]));
    WriteLineToLog(Format('   - Shadow copy device name: %s',
      [m_pwszSnapshotDeviceObject]));
    WriteLineToLog(Format('   - Originating machine: %s',
      [Prop.m_pwszOriginatingMachine]));
    WriteLineToLog(Format('   - Service machine: %s',
    ...
    else if m_lSnapshotAttributes and VSS_VOLSNAP_ATTR_EXPOSED_REMOTELY <> 0
    then
    begin
      WriteLineToLog(Format('   - Exposed remotely as %s',
        [m_pwszExposedName]));
      if length(m_pwszExposedPath) > 0 then
        WriteLineToLog(Format('   - Path exposed: %s', [m_pwszExposedPath]))
      else
        WriteLineToLog('   - Not Exposed');
    end;
    WriteLineToLog(Format('   - Provider id: %s',
      [GuidToString(m_ProviderId)]));
  end;
  ...
  if lAttributes and VSS_VOLSNAP_ATTR_NO_AUTO_RELEASE <> 0 then
    ...
    attributes := attributes + ' Auto_Release';
...
begin
  WriteLineToLog('');
  if not IsVssAvailable then
  begin
    raise EVssError.Create(rsVssNotAvail);
  end;
  ShowStatusMsg(rsInitBackupComps);
  // Initialize COM
  ...
  if not VssCoInitializeSecurityCalled then
  begin
  ...
  // nil // Reserved parameter
  // ), 'TVolumeShadowCopy.Initialize:CoInitializeSecurity');
  ...
  if DuringRestore then
    OleCheck(VssBackupComponents.InitializeForRestore(PChar(AXmlDoc)),
  ...
  begin
    ...
    if length(AXmlDoc) = 0 then
      ...
      OleCheck(VssBackupComponents.InitializeForBackup(PChar(AXmlDoc)),
        'TVolumeShadowCopy.Initialize:VssBackupComponents.InitializeForBackup')
  end;
  // Keep the context
  VssContext := AContext;
...
begin
  ...
  OleCheck(hrReturned,
    'TVolumeShadowCopy.WaitAndCheckForAsyncOperation:Async.QueryStatus:hrReturned');
end;
...
begin
  ...
  // Enumerate writers
  for iWriter := 0 to cWriters - 1 do
  begin
    // Get the metadata for this particular writer
    idInstance := GUID_NULL;
    OleCheck(VssBackupComponents.GetWriterMetadata(iWriter, idInstance,
      pMetadata),
...
var
  Async: IVssAsync;
begin
  ShowStatusMsg(rsGatherMetaData);
...
begin
  ...
  for i := 0 to WriterList.Count - 1 do
    with WriterList[i] as TVssWriter do
      if AnsiSameText(finstanceId, InstanceId) and not IsExcluded then
      begin
        Result := true;
        Exit;
      end;
...
var
  pAsync: IVssAsync;
begin
  ShowStatusMsg(rsGatherWriterStatus);
  ...
  begin
    ...
    if cWriters > 0 then
      ...
      begin
        ...
        if not IsWriterSelected(idInstance) then
          continue;
        ...
        case eWriterStatus of
          VSS_WS_FAILED_AT_IDENTIFY, VSS_WS_FAILED_AT_PREPARE_BACKUP,
            ...
            begin
              WriteLineToLog
                (Format('ERROR : Selected writer "%s" is in failed state!' +
                sLineBreak + 'Status: %d (%s)' + sLineBreak +
                'Writer Failure code : $%.8x' + sLineBreak + 'Writer ID : {%s}'
                + sLineBreak + 'Instance ID : {%s}',
                [fWriterName, integer(eWriterStatus),
                GetStringFromWriterStatus(eWriterStatus), hrWriterFailure,
...
begin
  ...
  begin
    ...
    OleCheck(VssBackupComponents.GetWriterStatus(iWriter, idInstance, idWriter,
      bstrWriterName, eWriterStatus, hrWriterFailure));
    // Print writer status
    WriteLineToLog(Format(sLineBreak + '* WRITER "%s"' + sLineBreak +
      '   - Status: %d (%s)' + sLineBreak +
      '   - Writer Failure code: $%.8x (%s)' + sLineBreak + '   - Writer ID: %s'
      + sLineBreak + '   - Instance ID: %s', [bstrWriterName,
      integer(eWriterStatus), GetStringFromWriterStatus(eWriterStatus),
      hrWriterFailure, HResultToString(hrWriterFailure), GuidToString(idWriter),
      GuidToString(idInstance)]));
  end;
end;

// Discover directly excluded components (that were excluded through the command-line)
procedure TVolumeShadowCopy.DiscoverDirectlyExcludedComponents
  (ExcludedWriterAndComponentList: TStringList; AWriterList: TObjectList);
var
...
begin
  ShowStatusMsg(rsDscvDirExclComps);
  ...
  for iWriter := 0 to AWriterList.Count - 1 do
  begin
    fWriter := AWriterList[iWriter] as TVssWriter;
    // Check if the writer is excluded
    with ExcludedWriterAndComponentList do
    ...
    begin
      ...
      for iComponent := 0 to fWriter.Components.Count - 1 do
      begin
        ...
        componentPathWithWriterName := fWriter.WriterName + ':' +
          fComponent.FullPath;
        componentPathWithWriterID := fWriter.WriterId + ':' +
          fComponent.FullPath;
        componentPathWithWriterIID := fWriter.InstanceId + ':' +
          fComponent.FullPath;
        // Check to see if this component is explicitly excluded
        with ExcludedWriterAndComponentList do
        ...
        begin
          WriteLineToLog
            (Format('- Component "%s" from writer "%s" is explicitly excluded from backup',
      ...
      begin
        fComponent := fWriter.Components[iComponent] as TVssComponent;
        if not fComponent.IsExcluded then
          nonExcludedComponents := true;
      end;
      // If all components are missing or excluded, then exclude the writer too
      if not nonExcludedComponents then
      begin
        WriteLineToLog
          (Format('- Excluding writer "%s" since it has no selected components for restore.',
          [fWriter.WriterName]));
        fWriter.IsExcluded := true;
      end;
    end;
...
procedure TVolumeShadowCopy.DiscoverNonShadowedExcludedComponents
  (ShadowSourceVolumes: TStringList);
var
  ...
  wsLocalVolume: string;
begin
  ShowStatusMsg(rsDscvNsExclComps);
  // Discover components that should be excluded from the shadow set
  // This means components that have at least one File Descriptor requiring
  // volumes not in the shadow set.
  for iWriter := 0 to WriterList.Count - 1 do
  begin
    ...
    // Check if the writer is excluded
    if not fWriter.IsExcluded then
    begin
      // Check if the component is excluded
      for iComponent := 0 to fWriter.Components.Count - 1 do
      begin
        ...
        begin
          ...
          for iVol := 0 to fComponent.AffectedVolumes.Count - 1 do
          begin
            if (ClusterIsPathOnSharedVolume(PWideChar(fComponent.AffectedVolumes
              [iVol]))) then
            begin
              ClusterGetVolumeNameForVolumeMountPoint
                (PWideChar(fComponent.AffectedVolumes[iVol]), wsUniquePath,
                MAX_PATH + 1);
              fComponent.AffectedVolumes[iVol] := wsUniquePath;
            end;
            if ShadowSourceVolumes.IndexOf(fComponent.AffectedVolumes[iVol]) < 0
            then
            begin
              if (GetDisplayNameForVolume(fComponent.AffectedVolumes[iVol],
                wsLocalVolume)) then
                WriteLineToLog
                  (Format('- Component "%s" from writer "%s" is excluded from backup (it requires "%s" in the shadow set)',
                  [fComponent.FullPath, fWriter.WriterName, wsLocalVolume]))
              else
                WriteLineToLog
                  (Format('- Component "%s" from writer "%s" is excluded from backup',
                  [fComponent.FullPath, fWriter.WriterName]));
              fComponent.IsExcluded := true;
              Break;
            end;
...
begin
  ShowStatusMsg(rsDscvAllExclComps);
  // Discover components that should be excluded from the shadow set
  // This means components that have at least one File Descriptor requiring
  // volumes not in the shadow set.
  for iWriter := 0 to WriterList.Count - 1 do
  begin
    fWriter := WriterList[iWriter] as TVssWriter;
    if not fWriter.IsExcluded then
    begin // Ignore explicitly excluded writers
      ...
      begin
        ...
        // If yes, deselect it
        for i := 0 to fWriter.Components.Count - 1 do
        begin
          descendent := fWriter.Components[i] as TVssComponent;
          if fComponent.IsAncestorOf(descendent) and descendent.IsExcluded then
          begin
            WriteLineToLog
              (Format('- Component "%s" from writer "%s" is excluded from backup '
              + '(it has an excluded descendent: %s)', [fComponent.FullPath,
              fWriter.WriterName, descendent.CompName]));
            fComponent.IsExcluded := true;
            Break;
          end;
        end;
...
var
  ...
  fComponent: TVssComponent;
begin
  ShowStatusMsg(rsDscvExclWriters);
  // Enumerate writers
  for iWriter := 0 to WriterList.Count - 1 do
  begin
    fWriter := WriterList[iWriter] as TVssWriter;
    ...
    begin // Ignore explicitly excluded writers
      ...
      begin
        fComponent := fWriter.Components[iComponent] as TVssComponent;
        if fComponent.CanBeExplicitlyIncluded then
        begin
      ...
      if fWriter.IsExcluded then
      begin
        WriteLineToLog
          (Format('- The writer "%s" is now entirely excluded from the backup:',
          [fWriter.WriterName]));
        WriteLineToLog
      ...
      begin
        ...
        begin
          fComponent := fWriter.Components[iComponent] as TVssComponent;
          if fComponent.IsTopLevel and not fComponent.IsSelectable and
            fComponent.IsExcluded then
          begin
            WriteLineToLog
              (Format('- The writer "%s" is now entirely excluded from the backup:',
              [fWriter.WriterName]));
            WriteLineToLog
              (Format('  (the top-level non-selectable component "%s" is an excluded component)',
              [fComponent.FullPath]));
            fWriter.IsExcluded := true;
            Break;
          end;
...
begin
  ShowStatusMsg(rsDscvExpInclComps);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if ancestor.IsAncestorOf(fComponent) and ancestor.CanBeExplicitlyIncluded
            then
            begin
...
procedure TVolumeShadowCopy.VerifyExplicitelyIncludedComponent
  (const AIncludedComponent: string; AWriterList: TObjectList);
var
...
begin
      ...
      begin
        ...
        begin
          ...
          if (AnsiSameText(componentPathWithWriterName, AIncludedComponent) or
            ...
            AnsiSameText(componentPathWithWriterIID, AIncludedComponent)) then
          begin
            // If we are during restore, we just found our component
            if DuringRestore then
            begin
              WriteLineToLog(Format(' - The component "%s" is selected',
                [AIncludedComponent]));
              Exit;
            end;
            ...
            begin
              for k := 0 to fWriter.Components.Count - 1 do
              begin
                ancestor := fWriter.Components[k] as TVssComponent;
                if ancestor.IsAncestorOf(fComponent) and ancestor.IsExplicitlyIncluded
                then
                begin
                  isIncluded := true;
                  Break;
            ...
            begin
              WriteLineToLog
                (Format('ERROR : The component "%s" was not included in the backup! Aborting backup ...',
                [AIncludedComponent]));
              WriteLineToLog
                ('- Please reveiw the component/subcomponent definitions');
              WriteLineToLog
                ('- Also, please verify list of volumes to be shadow copied.');
  ...
  WriteLineToLog
    (Format('ERROR : The component "%s" was not found in the writer components list! Aborting backup ...',
    [AIncludedComponent]));
...
procedure TVolumeShadowCopy.VerifyExplicitelyIncludedWriter(const AWriterName
  : string; AWriterList: TObjectList);
var
  iWriter, iComponent: integer;
  fWriter: TVssWriter;
...
begin
  WriteLineToLog
    (Format('- Verifing that all components of writer "%s" are included in backup ...',
    [AWriterName]));
  // Enumerate all writers
  for iWriter := 0 to WriterList.Count - 1 do
  begin
    ...
    begin // Ignore explicitly excluded writers
      ...
      if AnsiSameText(AWriterName, fWriter.WriterName) or
        ...
        AnsiSameText(AWriterName, fWriter.InstanceId) then
      begin
        if fWriter.IsExcluded then
        begin
          WriteLineToLog
            (Format('ERROR : The writer "%s" was not included in the backup! Aborting backup ...',
            [AWriterName]));
          WriteLineToLog
            ('- Please review the component/subcomponent definitions');
          WriteLineToLog
            ('- Also, please verify list of volumes to be shadow copied.');
          raise EOleSysError.Create('', E_INVALIDARG, 0);
        end;
        ...
        begin
          fComponent := fWriter.Components[iComponent] as TVssComponent;
          if fComponent.IsExcluded then
          begin
            WriteLineToLog
              (Format('ERROR : The writer "%s" has components not included in the backup! Aborting backup ...',
              [AWriterName]));
            WriteLineToLog
              (Format('- The component "%s" was not included in the backup.',
              [fComponent.FullPath]));
            WriteLineToLog
              ('- Please reveiw the component/subcomponent definitions');
            WriteLineToLog
        ...
        WriteLineToLog
          (Format('  - All components from writer "%s" are selected',
          [AWriterName]));
        Exit;
      end;
    end;
  ...
  WriteLineToLog
    (Format('"ERROR : The writer \"%s\" was not found! Aborting backup ...',
    [AWriterName]));
...
var
  ...
  fComponent: TVssComponent;
begin
  ShowStatusMsg(rsSelExpInclComps);
  // Enumerate all writers
  for iWriter := 0 to WriterList.Count - 1 do
  begin
    fWriter := WriterList[iWriter] as TVssWriter;
    ...
    begin // Ignore explicitly excluded writers
      ...
      begin
        ...
        begin
          ...
          // Add the component
          OleCheck(VssBackupComponents.AddComponent
...
begin
  ...
  if assigned(ExcludedWriterAndComponentList) then
    DiscoverDirectlyExcludedComponents(ExcludedWriterAndComponentList,
      WriterList);
  ...
  if assigned(IncludedWriterAndComponentList) then
  begin
    // Verify if the specified writers/components were included
    WriteLineToLog('Verifying explicitly specified writers/components ...');
    for i := 0 to IncludedWriterAndComponentList.Count - 1 do
    begin
      // Check whether a component or a writer is specified
      if Pos(':', IncludedWriterAndComponentList[i]) <> 0 then
        VerifyExplicitelyIncludedComponent(IncludedWriterAndComponentList[i],
          WriterList)
      else
        VerifyExplicitelyIncludedWriter(IncludedWriterAndComponentList[i],
          WriterList);
    end;
  end;
...
procedure TVolumeShadowCopy.SaveBackupComponentsDocument
  (const AFileName: string);
var
  bstrXML: string;
begin
  ShowStatusMsg(rsSaveBackCompsDoc);
...
var
  ...
  snapshotID: TGuid;
begin
  ShowStatusMsg(rsAddVolSnapSet);
  // Preserve the list of volumes for script generation
  LatestVolumeList := VolumeList; // never used ???
  Assert(length(FLatestSnapshotIdList) = 0,
    'ASSERTION FAILED : No empty list of latest snapshots');
  // Add volumes to the shadow set
  for i := 0 to VolumeList.Count - 1 do
  begin
    volume := VolumeList[i];
    if not GetDisplayNameForVolume(volume, s) then
      s := '??';
    WriteLineToLog(Format('- Adding volume %s [%s] to the shadow set...',
      [volume, s]));
    OleCheck(VssBackupComponents.AddToSnapshotSet(PWideChar(volume), GUID_NULL,
      snapshotID),
      'TVolumeShadowCopy.AddToSnapshotSet:VssBackupComponents.AddToSnapshotSet');
    // Preserve this shadow ID for script generation
    AddGuidToList(FLatestSnapshotIdList, snapshotID);
  end;
end;
...
var
  pAsync: IVssAsync;
begin
  ShowStatusMsg(rsCreateShadowCopy);
  ...
  // Check selected writer status
  CheckSelectedWriterStatus;
  ShowStatusMsg(rsCreateCopySuccess);
  WriteLineToLog('');
end;
...
var
  ...
  ulFetched: ULONG;
begin
  if SnapshotSetID = GUID_NULL then
    ShowStatusMsg(rsQueryShadowCopies + sLineBreak)
  else
    ShowStatusMsg(Format(rsQuerySnapshotSetID + sLineBreak,
      [GuidToString(SnapshotSetID)]));
  // Get list all shadow copies.
  hr := VssBackupComponents.Query(GUID_NULL, VSS_OBJECT_NONE,
    VSS_OBJECT_SNAPSHOT, pIEnumSnapshots);
  ...
  begin
    if SnapshotSetID = GUID_NULL then
    begin
      WriteLineToLog(sLineBreak + 'There are no shadow copies in the system' +
        sLineBreak);
      Exit;
    end;
  end;
  try
    ...
    begin
      ...
      if (SnapshotSetID = GUID_NULL) or
        (Prop.PObj.Snap.m_SnapshotSetId = SnapshotSetID) then
        PrintSnapshotProperties(Prop.PObj.Snap);
    end;
  finally
...
begin
  try
    ...
    OleCheck(VssBackupComponents.GetSnapshotProperties(SnapshotSetID, ASnap));
    // Print the properties of this shadow copy
    // PrintSnapshotProperties(ASnap);
  finally
    // VssFreeSnapshotProperties(@ASnap);
  end;
end;
...
begin
  ...
  if SnapshotWithWriters then
  begin
    // Gather writer metadata
    GatherWriterMetadata;
    // Select writer components based on the given shadow volume list
    SelectComponentsForBackup(AVolumeList, AExcludedWriterList,
      AIncludedWriterList);
    // Start the shadow set
    OleCheck(VssBackupComponents.StartSnapshotSet(FLatestSnapshotSetID),
      'TVolumeShadowCopy.CreateSnapshotSet:VssBackupComponents.StartSnapshotSet');
    ShowStatusMsg(Format(rsCreateShadowSet,
      [GuidToString(FLatestSnapshotSetID)]));
    ...
    if VssContext and VSS_VOLSNAP_ATTR_DELAYED_POSTSNAPSHOT <> 0 then
    begin
      WriteLineToLog(sLineBreak + 'Fast snapshot created. Exiting... ' +
        sLineBreak);
      Exit;
    end;
    // Saves the backup components document, if needed
    if length(AOutXmlFile) > 0 then
      SaveBackupComponentsDocument(AOutXmlFile);
    // List all the created shadow copies
    if VssContext and VSS_VOLSNAP_ATTR_TRANSPORTABLE = 0 then
    begin
      WriteLineToLog(sLineBreak + 'List of created shadow copies:');
      QuerySnapshotSet(FLatestSnapshotSetID);
    end;
  end;
end;
...
begin
  ...
  if cWriters = 0 then
    WriteLineToLog('- There were no writer components in this backup')
  else
  begin
    if succeeded then
      ...
      WriteLineToLog
        ('- Backup failed. Mark all writers as not successfully backed up ... ');
    SetBackupSucceeded(succeeded);
    ShowStatusMsg(sLineBreak + rsCompleteBackup);
...
begin
  ...
  try
    ...
    for i := 0 to High(FLatestSnapshotIdList) do
    begin
      // Get shadow copy device (if the snapshot is there)
      if VssContext and VSS_VOLSNAP_ATTR_TRANSPORTABLE = 0 then
      begin
        OleCheck(VssBackupComponents.GetSnapshotProperties(FLatestSnapshotIdList
          [i], Snap),
...
begin
  ...
  try
    ...
    WinCheck(WaitForSingleObject(pi.hProcess, INFINITE) = WAIT_OBJECT_0,
      'WaitForSingleObject');
    ...
    begin
      WriteLineToLog
        (Format('ERROR : Command line "%s" failed!. Aborting the backup ...',
        [Command]));
...
procedure TVolumeShadowCopy.FreeSnapshotProperties(ASnap: TVssSnapshotProp);
begin
  VssFreeSnapshotProperties(@ASnap);
end;
...
begin
  ...
  hr := VssBackupComponents.Query(GUID_NULL, VSS_OBJECT_NONE,
    VSS_OBJECT_SNAPSHOT, pIEnumSnapshots);
  OleCheck(hr,
    'TVolumeShadowCopy.DeleteAllSnapshots:VssBackupComponents.Query');
  // If there are no shadow copies, just return
  ...
  begin
    try
      ...
      begin
        ...
        hr := pIEnumSnapshots.Next(1, Prop, ulFetched);
        OleCheck(hr,
          'TVolumeShadowCopy.DeleteAllSnapshots:pIEnumSnapshots.Next');
        // We reached the end of list
        if ulFetched = 0 then
          Break;
        // Print the deleted shadow copy...
        with Prop.PObj.Snap do
        begin
          WriteLineToLog
            (Format('- Deleting shadow copy "%s" on %s from provider "%s" [$%.8x] ...',
            [GuidToString(m_SnapshotId), m_pwszOriginalVolumeName,
            GuidToString(m_ProviderId), m_lSnapshotAttributes]));
          // Perform the actual deletion
          hr := VssBackupComponents.DeleteSnapshots(m_SnapshotId,
        ...
        begin
          ...
          WriteLineToLog
            (Format('- Last shadow copy that could not be deleted: "%s"',
            [GuidToString(idNonDeletedSnapshotID)]));
          OleCheck(hr,
            'TVolumeShadowCopy.DeleteAllSnapshots:VssBackupComponents.DeleteSnapshots');
        end;
      end;
    finally
      VssFreeSnapshotProperties(@Prop.PObj.Snap);
    ...
    WriteLineToLog(sLineBreak + 'There are no shadow copies on the system' +
      sLineBreak);
end;
...
begin
  // Print the deleted shadow copy...
  WriteLineToLog(Format('- Deleting shadow copy set "%s" ...',
    [GuidToString(SnapshotSetID)]));
  // Perform the actual deletion
  hr := VssBackupComponents.DeleteSnapshots(SnapshotSetID,
  ...
  begin
    ...
    WriteLineToLog(Format('- Last shadow copy that could not be deleted: "%s"',
      [GuidToString(idNonDeletedSnapshotID)]));
    OleCheck(hr,
      'TVolumeShadowCopy.DeleteSnapshotSet:VssBackupComponents.DeleteSnapshots');
  end;
end;

procedure TVolumeShadowCopy.DeleteOldestSnapshot(const stringVolumeName
  : string);
var
  uniqueVolume: string;
  pIEnumSnapshots: IVssEnumObject;
  ...
  Prop: TVssObjectProp;
  ulFetched: ULONG;
  OldestAttributes, lSnapshots: LONG;
  OldestSnapshotId, OldestProviderId, idNonDeletedSnapshotID: TGuid;
...
begin
  ...
  hr := VssBackupComponents.Query(GUID_NULL, VSS_OBJECT_NONE,
    VSS_OBJECT_SNAPSHOT, pIEnumSnapshots);
  OleCheck(hr,
    'TVolumeShadowCopy.DeleteOldestSnapshot:VssBackupComponents.Query');
  // If there are no shadow copies, just return
  if succeeded(hr) then
  begin
    ...
    try
      ...
      begin
        ...
        hr := pIEnumSnapshots.Next(1, Prop, ulFetched);
        OleCheck(hr,
          'TVolumeShadowCopy.DeleteOldestSnapshot:pIEnumSnapshots.Next');
        // We reached the end of list
        if ulFetched = 0 then
          Break;
        with Prop.PObj.Snap do
          if AnsiSameText(m_pwszOriginalVolumeName, uniqueVolume) and
            (OldestSnapshotTimestamp > m_tsCreationTimestamp) then
          begin
      ...
      if OldestSnapshotId <> GUID_NULL then
      begin
        // Print the deleted shadow copy...
        WriteLineToLog
          (Format('- Deleting shadow copy "%s" on %s from provider "%s" [$%.8x] ...',
        ...
        // Perform the actual deletion
        hr := VssBackupComponents.DeleteSnapshots(OldestSnapshotId,
          VSS_OBJECT_SNAPSHOT, false, lSnapshots, idNonDeletedSnapshotID);
        if Failed(hr) then
        begin
          WriteLineToLog('Error while deleting shadow copies ...');
...
{ TVssThread ------------------------------------------------------------------- }
constructor TVssThread.Create(const ADrive: string; ASuspend: boolean;
...
VssCoInitializeSecurityCalled := false;
